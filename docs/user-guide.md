...
`duration_s`, `n/a`, `restamp`) with a `TOTAL` row, where `n/a` counts events
whose `tokens` field is `null` (e.g. adapter-path runs, which record
duration only) so an untracked group reads as "no data", not a real zero,
and `restamp` counts events with verdict `RESTAMP_PASS`/`RESTAMP_FAIL`
(#258) — a cheap delta re-review of a PR the same role already approved —
separately from that group's full-stage events/tokens (see
"Stale approvals — cheap delta re-stamp" above).
