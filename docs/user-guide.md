...
| `""` (empty) | Same as `none`, but each `assign-issue` prints a one-line notice on stderr saying the empty value was read as `none`. An empty value is treated as "switched off" rather than `self` because assigning is a visible write on someone's board. To get `self`, remove the key. |
