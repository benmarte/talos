...
# entry is validated against a hard-coded non-unionable set (scripts/**,
# tests/**, pipeline config filenames) BEFORE it is used -- a union merge
# blindly concatenates both sides of a conflict, which is a safe operation
# for an additive changelog but would silently corrupt a source file. Config
# can never widen a union past these paths.
...
for entry in entries:
    if is_hardcoded_nonunionable(entry):
        errors.append(f\"merge.union_paths entry '{entry}' is a hard-coded non-unionable path\")
        continue
    for canary in VALIDATION_CANARIES:
        base = canary.rsplit('/', 1)[-1]
        if fnmatch.fnmatch(base, entry) or fnmatch.fnmatch(canary, entry):
            errors.append(f\"merge.union_paths entry '{entry}' would union '{canary}' -- rejected (catch-all or covers non-unionable paths)\")
            break
...
_mb_cleanup() {
  ...
  git worktree remove --force "$_MB_TMPDIR" >/dev/null 2>&1
...
if ! with_lock "$(git rev-parse --git-common-dir 2>/dev/null || echo .git)/talos-worktree" 10 -- \
...
if [ "$_MB_MERGE_RC" -eq 1 ]; then
  ...
  # Every conflicting path must match a union glob -- caps the loop at the
  # actual conflict count (no unbounded input; comes straight from git).
  _MB_NONUNION="$(CONFLICTS="$_MB_CONFLICTS" UNION_JSON="$_MB_UNION_PATHS" python3 -c "
...
")"
  if [ -n "$_MB_NONUNION" ]; then
    echo "pipeline-mergebase: conflicting path(s) not covered by merge.union_paths, not mechanically resolvable:" >&2
    printf '%s\n' "$_MB_NONUNION" >&2
    exit 3
  fi
