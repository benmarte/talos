...
When stamping clean, post the marker in a PR comment immediately after the security comment. Obtain the SHA from the API — never from `git rev-parse HEAD`, which returns main in this stage's working directory:
