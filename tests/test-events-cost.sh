...
assert_contains "$out" "$(printf '43\tdeveloper\t1\t0\t0\t30\t1')" "cost: issue 43 / developer row (null tokens summed as 0)"
assert_contains "$out" "$(printf 'TOTAL\t\t4\t350\t8\t65\t1')" "cost: TOTAL row sums every group"
...
assert_contains "$out" "$(printf 'TOTAL\t\t3\t350\t8\t35\t0')" "cost --issue 42: TOTAL row scoped to issue 42 only"
...
_na_count="$(printf '%s' "$_na_line" | awk -F'\t' '{ print $NF }')"
