...
**(b) `pipeline-events.sh cost`'s `n/a` column is now `unrecorded`, and `tokens` may be `null` (#259).** The JSON key and table header previously named `n/a` for a stage whose usage was never recorded are now `unrecorded`, and the `--json` `tokens` field for such a stage is `null` rather than a number. Any script or dashboard consuming `pipeline-events.sh cost --json` must tolerate a `null` `tokens` value and the renamed `unrecorded` key — this is expected for reviewer/security/validator/docs stages on the native subagent path, not a bug.
