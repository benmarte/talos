...
| `pr-files` | `<pr-number>` | Print the PR's changed paths, one per line — no filtering or exit-1 gate (unlike `check-pr-files`); reuses the same file listing `check-pr-files` already fetches. Used by the Step 3e Phase 1 `roles.docs_mode: auto` gate (#200) to decide whether the docs stage needs to dispatch at all. GitHub provider only (`github`/`github-api` parity); `gitlab`, `azure`, and `file` fail open with a stderr warning and empty stdout. |
