...
| `issues.assignee` | `self` | Who `create-issue` and the "In progress" claim assign an issue to (github, github-api, gitlab, azure; not file). `self` = the authenticated operator; any other value = that identity verbatim; `none` = never assign; `""` (key present but empty) = `none`, plus a one-line stderr notice (remove the key for `self`). An existing assignee is never overwritten, and an identity the provider rejects is a stderr warning, never a stage failure. See [docs/user-guide.md](docs/user-guide.md#who-issues-are-assigned-to-issuesassignee). |
