...
| `issues.assignee` | `self` | Who `create-issue` and the "In progress" claim assign an issue to (github, github-api, gitlab, azure; not file). `self` = the authenticated operator; any other value = that identity verbatim; `none` = never assign; `assignee: ""` (QUOTED empty string) = `none`, plus a one-line stderr notice; a bare `assignee:` (YAML null) behaves as if unset, resolving to `self`. An existing assignee is never overwritten, and an identity the provider rejects is a stderr warning, never a stage failure. See [docs/user-guide.md](docs/user-guide.md#who-issues-are-assigned-to-issuesassignee). |
...
| `check-pr-files` | `<pr-number>` | Exit 1 if the PR touches `merge.forbidden_files` patterns. On `github`/`github-api`/`gitlab` (#303) a failed fetch also exits 1 (fail closed). Not implemented on `azure` or `file`: they print a stderr warning and exit 0, so this gate does not protect those providers. |
