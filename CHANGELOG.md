...
- **`scripts/pipeline-vcs.sh` `check-approval-sha`: reject approval markers whose `role=` value is not in the canonical set (#128).** The marker regex previously accepted any non-whitespace token in the role position. A marker such as `<!-- talos:approval sha=<SHA> role=review -->` (using `review` instead of `reviewer`) or any other unrecognised value was treated as a valid marker and counted toward the approval gate. Fix: a `VALID_ROLES = {'qa', 'reviewer', 'security', 'docs'}` constant is added to both the `_github` and `_github_api` provider copies of the `check-approval-sha` Python block. After the regex matches, the role value is validated against this fixed literal set before the per-label equality check. An unrecognised role emits a log line to stderr (`ignoring marker with unknown role <role> (valid: docs, qa, reviewer, security)`) and is skipped; the gate then falls through to the existing `STALE -- no SHA marker` exit (fail-closed). The valid set is a hard-coded literal, never interpolated from config or API text (PR #68 precedent). Reason strings are ASCII-only and byte-identical across both providers. **What this PR does NOT close:** dispatch provenance (a marker with no mandate -- an agent can still post all four markers with valid roles against the current head SHA and satisfy the gate) and contradictory verdicts (the marker format has no verdict field; a PASS marker alongside a FAIL prose comment is invisible to any marker-only scan). Both gaps remain open and require a separate design. Four new test cases in `tests/test-approval-sha.sh` and three in `tests/test-github-api.sh` cover the unknown-role log-line assertion, each individual valid role, the full four-role regression guard, and the stale-SHA path; all showed RED before the fix from wrong behaviour (missing log line or wrong exit), not from absence.
