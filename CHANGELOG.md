...
- **Epic auto-close sweep no longer closes an epic on its children's completion alone -- it now gates on the epic's own acceptance boxes (#168).** Step 1.6 closed an epic once every `Part of #<E>` sub-issue was closed, without ever checking the epic's own `- [ ]` acceptance criteria; a planner that dropped or under-scoped a criterion left it silently abandoned while the epic closure read as success. Fixed with a new `check-epic-acceptance <N>` verb (`scripts/pipeline-vcs.sh`, implemented in both `_github` and `_github_api`) that scans the epic's body for unticked `- [ ] ` checklist lines: exits 0 (including when the body has no checkboxes at all) when none remain, exits non-zero and prints each one's text when any remain. The sweep now calls it before closing: exit 0 closes as before; exit non-zero adds the new `pipeline:epic-children-done` label (`scripts/bootstrap-labels.sh`) and comments naming every outstanding item instead of closing, leaving the epic open for a human. `gitlab`/`azure`/`file` providers fail open on the new verb (consistent with their existing best-effort pattern for other GitHub-only checks), so the sweep degrades to prior behaviour there rather than hard-failing. `tests/test-epic-acceptance.sh` (23 assertions, RED on unpatched `pipeline-vcs.sh`) covers both providers, dry-run, and the unticked/all-ticked/no-checkboxes cases; `tests/test-e2e-pipeline.sh` adds a sweep scenario proving an epic with unticked boxes stays open and labelled while one with all boxes ticked (or none) still closes.
