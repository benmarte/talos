...
- **`list-issues`/`list-prs` no longer silently truncate a backlog bigger than 100 items (#171).** The `github` provider hardcoded `gh issue list`/`gh pr list` at a `--limit` of 100 (issues) or the CLI default of 30 (PRs, no `--limit` at all) with no override; the `github-api` provider capped both at a single 100-item page. `github` now requests `--limit 1000` -- GitHub's own ceiling on search-API result sets, and the highest bound `gh` can honor for either command -- and warns loudly on stderr (`WARNING result capped at 1000`) if a result lands exactly on it, since more items may exist beyond what that request can return. `github-api` now paginates both endpoints fully by following `Link: rel="next"` headers until exhausted (`_ga_fetch_all_pages`, generalized from the existing comment-pagination helper), so it is no longer capped at all; a page that fails partway through exits non-zero with no output, rather than returning a truncated list as if it were complete. `gitlab` and `azure` have no CLI-level "fetch every page" mechanism (`glab` has no `--paginate` equivalent for these commands; `az boards query` has no `--top`/page flag at all), so they stay capped -- at `glab`'s own `--per-page` ceiling of 100, and at an explicit `SELECT TOP 1000/--top 1000` for azure -- but now warn loudly on stderr naming the exact cap whenever a result lands on it, instead of failing silently. New tests across `tests/test-vcs.sh`, `tests/test-github-api.sh`, and `tests/test-providers.sh` cover a 150-item fixture returning whole for `github`/`github-api`, a failed second page exiting non-zero with no partial output, and the cap-warning firing for `github`/`gitlab`/`azure` and staying silent under the cap.
