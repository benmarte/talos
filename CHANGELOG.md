...
- **`.github/workflows/tests.yml`: skip CI on draft PRs; skip CI for markdown-only pull requests (#145).** Draft PRs no longer trigger the test jobs (`if: github.event.pull_request.draft != true`); marking a draft ready triggers them on both runners. `paths-ignore: ['**.md']` is scoped to the `pull_request` trigger only — a markdown-only PR does not run CI, but pushes to `main` always run (the `push:` trigger carries no `paths-ignore`). NOTE: if required status checks are ever added for merge, a no-op job matching `test (ubuntu-latest)` / `test (macos-latest)` will be needed or markdown-only PRs will hang "expected -- waiting".
