...
- **`scripts/pipeline-vcs.sh` `check-approval-sha`: normalise STALE reason punctuation to ASCII `--` in both providers (#127).** Three error paths in the `_github` Python block used em dash (U+2014) where `_github_api` used ASCII `--`: the no-marker path (`'has no SHA marker'`), the abbreviated-SHA rejection, and the nonexistent-SHA rejection. The fix normalises `_github` to ASCII `--` rather than promoting `_github_api` to em dash. ASCII is the correct target because `_github_api` Python blocks run inside double-quoted bash strings and therefore cannot use f-strings; all literal text must be written as hand-concatenated strings. A non-ASCII literal in that context is prone to silent narrowing (the original `--` → `--` divergence was caused by exactly this: an f-string edit in `_github` that `_github_api` could not safely mirror). Normalising to ASCII removes the failure mode instead of instrumenting it. Two new cross-provider regression tests in `tests/test-approval-sha.sh` capture the STALE reason line from both providers for the abbreviated-SHA and nonexistent-SHA paths and assert byte-equality (`assert_eq`), so any future punctuation divergence between providers fails loudly.
