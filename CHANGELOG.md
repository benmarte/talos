...
- **`scripts/pipeline-vcs.sh` `_github_api` provider: verb parity with `_github` (#75).** Six verbs that existed only in the `gh`-backed `_github` provider are now implemented in the REST-only `_github_api` provider: `pr-head` (GET `/pulls/<n>`, returns `head.sha`), `read-attempt` / `check-attempt` / `record-attempt` (attempt-counting verbs backed by GET/POST `/issues/<n>/comments`), `check-approval-sha` (two REST calls — GET `/pulls/<n>` and GET `/issues/<n>/comments` — with `user.login` normalised to `author.login` before the shared Python decision block, yielding a byte-identical pass message `check-approval-sha: all approval labels are current`), and `check-closing-keyword` (GET `/pulls/<n>` + GET `/pulls?state=open` with `head.ref` normalised to `headRefName`). The `label-pr` approval-marker guards — both the `--require-marker` pre-flight check and the post-dispatch warning — now run for `github-api` in addition to `github`; the post-dispatch guard delegates to `bash "$0" check-approval-sha`. INVARIANT (issue #79) comments added in all four last-line-rule enforcement sites (both providers' `read-attempt` and `check-approval-sha` Python blocks). New `tests/test-verb-parity.sh` (4 tests) extracts verb sets from both providers by line-range, compares them, and includes a RED-proof mutation (fake-test-verb injected into `_github` only exits 1 printing "Only in _github: ['fake-test-verb']"). `tests/test-github-api.sh` extended from 135 to 198 tests covering all six new verbs: pass, fail, dry-run, exit-zero proof, `user.login` normalisation, byte-identical pass message, and `label-pr → check-approval-sha` routing.
