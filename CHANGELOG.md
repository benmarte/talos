...
- **`.github/workflows/tests.yml`: skip CI on draft PRs; skip CI for markdown-only changes (#145).** Draft PRs no longer trigger the test jobs (`if: github.event.pull_request.draft != true`); marking a draft ready triggers them on both runners. `paths-ignore: ['**.md']` skips runs for CHANGELOG/docs-only pushes. NOTE: if required status checks are ever added for merge, a no-op job matching `test (ubuntu-latest)` / `test (macos-latest)` will be needed or markdown-only PRs will hang "expected -- waiting".
