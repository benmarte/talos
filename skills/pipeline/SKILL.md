...
- **`subagents: true`** (native subagents, e.g. Claude Code) — spawn them as each stage instructs, after the per-role runner check above sends it here. **Per-role model selection (native path, `claude`-routed roles only):** Before spawning each subagent, resolve its model in three steps:
  ...
  **Per-role effort selection (native path, `claude`-routed roles only, #271):** resolve effort the same way, immediately after model:
  1. Read `agents.roles.<role>.effort` via `bash scripts/pipeline-config.sh agents.roles.<role>.effort`.
  2. If empty, read `agents.effort` via `bash scripts/pipeline-config.sh agents.effort`.
  3. If still empty, do nothing for this role — no `effort:` field is written or removed.

  Unlike model, there is no per-spawn Agent tool parameter for effort — do not invent one. When a non-empty value is found, apply it via the dispatched agent definition's frontmatter `effort:` field, the mechanism Claude Code exposes for subagents: only when this is a **repo-override** role (`$PWD/.claude/agents/<role>.md` exists — see "Subagent names" above), rewrite that file's `effort:` frontmatter line to the resolved value before spawning (add it right after `model:` if the file has no `effort:` line yet; leave every other line untouched). Skip this for a plugin/global-install role (no repo override present) — that agent definition is shared across projects and possibly concurrent runs, so Talos never mutates it; the role spawns with whatever effort its shipped definition already carries. Idempotent: re-running with the same resolved value is a no-op diff.
...
- Effort (#271): same chain shape, resolve `agents.roles.<role>.restamp_effort` via `bash scripts/pipeline-config.sh agents.roles.<role>.restamp_effort`, falling back to `agents.restamp_effort`, falling back to that role's already-resolved effort from the Per-role effort selection block above (`agents.roles.<role>.effort` → `agents.effort` → unset). Also resolved by `pipeline-config.sh` itself — a genuinely empty result falls through to the role's normal effort. Applied the same way as the role's normal effort (frontmatter `effort:` field on a repo-override role, `TALOS_EFFORT` on the adapter path) — a re-stamp is a cheap dispatch, not a different application mechanism.
