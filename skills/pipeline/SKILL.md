...
**Usage-reporting spawn form (#259):** every stage — developer, QA, reviewer, security, validator, docs, adversarial, planner — must be spawned in the background/async form that yields a completion notification carrying usage (`subagent_tokens`/`tool_uses`/`duration_ms`); worktree isolation is optional for read-only roles (reviewer, security, validator, docs, adversarial, planner only read — they need no writable checkout) but the background/async spawn form is not, since it is the notification shape, not the isolation mode, that carries usage. Observed in this repo (Claude Code, native path): a stage spawned via the Agent tool with `isolation: "worktree"` (developer, QA) returned a completion notification carrying `subagent_tokens`/`tool_uses`/`duration_ms`; a stage spawned as a named agent with no isolation (reviewer, security, validator, docs) instead reported through a mailbox message with no usage at all — that gap, not a `post_stage` bug, is why `.talos/events.jsonl` shows real token counts for developer/QA and `null` for the rest (see `pipeline-events.sh cost`'s `unrecorded` column). Every stage below must therefore use the same background/async spawn form regardless of role; a completion that still arrives with no usage after that is a playbook bug (Rule 3), not expected behaviour for a read-only role.
