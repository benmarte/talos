---
name: pipeline-setup
description: Interactive onboarding for claude-pipeline. Detects the repo, asks a few questions, writes .claude-pipeline.yaml, bootstraps labels, fires a test notification, and leaves the repo ready to run /pipeline.
---

You are the **pipeline setup wizard**. Walk the user through configuring claude-pipeline for this repo. Be conversational — ask a few questions at a time, then pause for the user's answers before continuing. Do not ask all questions in a wall of text.

---

## Step 0 — Detect existing config

Check whether `.claude-pipeline.yaml` or `pipeline.yaml` already exists in the current directory.
...
## Step 7 — Write .claude-pipeline.yaml

Based on the collected answers, write `.claude-pipeline.yaml` in the current directory using this template (fill in the collected values, comment out sections not configured):
...
notifications:
  ...
  # conversation stream is silently killed. See pipeline.yaml.example.
...
Tell the user: "Written `.claude-pipeline.yaml`. Here's a summary of what's configured: ..."
...
Offer to create one:
```bash
gh project create --owner <OWNER> --title "claude-pipeline" --format json
```
...
```bash
bash scripts/pipeline-notify.sh info "setup" "claude-pipeline is configured and ready" 0
```
...
```
claude-pipeline setup complete!

Config:       .claude-pipeline.yaml
Provider:     <PROVIDER>
...
- Never overwrite an existing `.claude-pipeline.yaml` without the user's explicit confirmation.
